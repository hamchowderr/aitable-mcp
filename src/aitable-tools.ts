...
async function aitableFetch<T>(
  ...
  // Check HTTP-level response status first
  if (!response.ok) {
...
// Register all AITable tools on the MCP server
export function registerAITableTools(server: McpServer, apiToken: string, spaceId: string) {
  // Get Records Tool
  server.tool(
    "get_records",
    ...
    async ({
      ...
      try {
        ...
        const result = await aitableFetch<GetRecordsResponse>(apiToken, endpoint);
  ...
  server.tool(
    ...
    async ({ datasheetId, records, viewId, fieldKey }): Promise<CallToolResult> => {
      try {
        ...
        const processedRecords = records.map((record, index) => {
          ...
          for (const [fieldName, value] of Object.entries(record.fields)) {
            ...
            // Check if value looks like an attachment array
            if (Array.isArray(value) && value.length > 0 && value[0]?.token) {
              // Validate each attachment has both token and name
  ...
  server.tool(
    ...
    async ({ datasheetId, recordIds }): Promise<CallToolResult> => {
      try {
        // Validate record IDs
  ...
  server.tool(
    ...
    async ({ datasheetId, viewId }): Promise<CallToolResult> => {
      try {
        const params = new URLSearchParams();
        if (viewId) params.append("viewId", viewId);
        ...
        const result = await aitableFetch<GetFieldsResponse>(apiToken, endpoint);
  ...
  server.tool(
    ...
    async ({ datasheetId, type, name, property }): Promise<CallToolResult> => {
      try {
        // Validate inputs
        if (!name || name.trim() === '') {
        ...
        const endpoint = `/spaces/${spaceId}/datasheets/${datasheetId}/fields`;
  ...
  server.tool(
    ...
    async ({ datasheetId, fieldId }): Promise<CallToolResult> => {
      try {
        const endpoint = `/spaces/${spaceId}/datasheets/${datasheetId}/fields/${fieldId}`;
  ...
  server.tool(
    ...
    async ({ datasheetId }): Promise<CallToolResult> => {
      try {
        ...
        const result = await aitableFetch<GetViewsResponse>(apiToken, endpoint);
  ...
  server.tool(
    ...
    {
      // No parameters needed - uses spaceId from environment
    },
    async (): Promise<CallToolResult> => {
      try {
        const endpoint = `/spaces/${spaceId}/nodes`;

        const result = await aitableFetch<GetNodeListResponse>(apiToken, endpoint);

        return {
          content: [
            {
              type: "text",
              text: `✓ Retrieved ${result.data?.nodes.length || 0} nodes from space ${spaceId}\n\n${JSON.stringify(result.data, null, 2)}`,
            },
          ],
        };
      } catch (error) {
  ...
  server.tool(
    ...
    async ({ name, description, folderId, preNodeId, fields }): Promise<CallToolResult> => {
      try {
        // Validate datasheet name
        if (!name || name.trim() === '') {
        ...
        const endpoint = `/spaces/${spaceId}/datasheets`;
  ...
  server.tool(
    ...
    async ({ datasheetId, filePath }): Promise<CallToolResult> => {
      try {
        // Validate inputs
        ...
        // Convert Buffer to Uint8Array for web Blob compatibility
        const fileBlob = new Blob([new Uint8Array(fileBuffer)]);

        // Create web FormData (not form-data package)
  ...
  server.tool(
    ...
    async ({ type, permissions, query }): Promise<CallToolResult> => {
      try {
        ...
        // Note: This is a v2 endpoint, so we need to use a different base URL
        const v2BaseUrl = "https://aitable.ai/fusion/v2";
        const endpoint = `/spaces/${spaceId}/nodes?${params.toString()}`;
  ...
  server.tool(
    ...
    async ({ nodeId, payload }): Promise<CallToolResult> => {
      try {
        const endpoint = `/spaces/${spaceId}/nodes/${nodeId}/embedlinks`;
  ...
  server.tool(
    ...
    async ({ nodeId }): Promise<CallToolResult> => {
      try {
        const endpoint = `/spaces/${spaceId}/nodes/${nodeId}`;

        const result = await aitableFetch<GetNodeDetailResponse>(apiToken, endpoint);

        // Format response message based on whether it's a folder or not
  ...
  server.tool(
    ...
    async ({ nodeId, linkId }): Promise<CallToolResult> => {
      try {
        const endpoint = `/spaces/${spaceId}/nodes/${nodeId}/embedlinks/${linkId}`;
